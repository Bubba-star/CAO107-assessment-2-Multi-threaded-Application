...
void loadImage(std::wstring filePath)
{
	...
	if (didLoadFail)
	{
		throw "oh no we failed to load";
	}

	imageVectorLock.lock();
	g_vecLoadedImages.push_back(loadedImage);
	imageVectorLock.unlock();
}
		...
		case ID_FILE_LOADIMAGE:
		{
			if (ChooseImageFilesToLoad(_hwnd))
			{
					...
					++imageFileNameIndex)
				{
					...
					threadPool.push_back(std::thread(loadImage, imageFileName));
				}
					...
					++imageFileNameIndex)
				{
					...
					RECT rect;
					rect.left = 0;
					bool success = SetRect(&rect, 200 * imageFileNameIndex, 0, 200 * imageFileNameIndex + 200, 200);
